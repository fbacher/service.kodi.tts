...
from gui.no_topic_models import NoSpinexTopicModel
from gui.parser.parse_spinex import ParseSpinex
from gui.spinex_topic_model import SpinexTopicModel
...
class SpinexModel(BaseModel):
    ...
    def __init__(self, parent: BaseModel, parsed_spinex: ParseSpinex) -> None:
        ...
        self.convert(parsed_spinex)

    def convert(self, parsed_spinex: ParseSpinex) -> None:
        """
            Convert Parsed Controls, etc. to a model.

        :param parsed_spinex: A ParseSpinex instance that
               needs to be converted to a SpinexModel
        :return:
        """
        clz = type(self)
        self.visible_expr = parsed_spinex.visible_expr
        self.description = parsed_spinex.description
        self.alt_label_expr = parsed_spinex.alt_label_expr
        self.on_focus_expr = parsed_spinex.on_focus_expr
        self.on_unfocus_expr = parsed_spinex.on_unfocus_expr
        self.enable_expr = parsed_spinex.enable_expr
        self.hint_text_expr = parsed_spinex.hint_text_expr
        self.action_expr = parsed_spinex.action_expr
        self.labeled_by_expr = parsed_spinex.labeled_by_expr
        self.info_expr = parsed_spinex.info_expr
        self.on_info_expr = parsed_spinex.on_info_expr

        if parsed_spinex.topic is not None:
            self.topic = SpinexTopicModel(self, parsed_spinex.topic)
        else:
            self.topic = NoSpinexTopicModel(self)

        clz._logger.debug(f'# parsed children: {len(parsed_spinex.get_children())}')
    ...
    def to_string(self, include_children: bool = False) -> str:
        ...
        results: List[str] = []
        result: str = (f'\nSpinexModel type: {self.control_type} '
        ...
        if include_children:
            for child in self.children:
                ...
                results.append(result)
        results.append('END SpinexModel')
        return '\n'.join(results)
