...
class Addon:
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.PATH: Path = Path(self.addon.getAddonInfo('path'))
        self.PROFILE = self.addon.getAddonInfo('profile')
        self.SUMMARY = self.addon.getAddonInfo('summary')
        self.TYPE = self.addon.getAddonInfo('type')
        self.MEDIA_PATH: Path = Path(self.PATH, "resources", "skins", "Default",
                                     "media")
        self.DATA_PATH: Path =\
            Path(xbmcvfs.translatePath(f"special://profile/addon_data/{self.ID}"))

    def setting(self, setting_name: str):
        """
    ...
    def set_password_prompt(self, setting_name):
        password = xbmcgui.Dialog().input(self.LANG(12326),
    ...
    def get_password(self, setting_name):
        ...
        if not self.addon.getSetting("mac_hash"):
            ...
            xbmcgui.Dialog().notification("Error",
                                          "MAC id changed. Please enter password"
                                          " again in settings.")
            self.addon.setSetting("mac_hash", mac_hash)
            return None
...
def encode_string(clear: str) -> bytes:
    enc = []
...
def decode_string(enc: bytes) -> str:
    """
