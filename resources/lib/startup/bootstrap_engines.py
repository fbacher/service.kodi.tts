...
from backends.settings.service_types import (EngineType, ServiceKey, Services,
                                             ServiceType,
                                             TTS_Type)
from common import *
...
class BootstrapEngines:
    ...
    def init(cls) -> None:
        ...
        if not cls._initialized:
            ...
            if MY_LOGGER.isEnabledFor(DEBUG):
                MY_LOGGER.debug(f'About to call SettingsLowLevel.load_settings')
    ...
    def configure_engine_settings(cls):
        ...
        try:
            ...
            if is_available:
                SettingsMap.set_available(Services.FESTIVAL_ID, Reason.AVAILABLE)
            else:
                SettingsMap.set_available(Services.FESTIVAL_ID, Reason.NOT_AVAILABLE)
        except AbortException:
            reraise(*sys.exc_info())
        except Exception as e:
            MY_LOGGER.exception('')
            SettingsMap.set_available(Services.FESTIVAL_ID, Reason.BROKEN)
        ...
        try:
            ...
            if is_available:
                SettingsMap.set_available(Services.FLITE_ID, Reason.AVAILABLE)
            else:
                SettingsMap.set_available(Services.FLITE_ID, Reason.NOT_AVAILABLE)
        except AbortException:
            reraise(*sys.exc_info())
        except Exception as e:
            MY_LOGGER.exception('')
        ...
        try:
            ...
            if is_available:
                SettingsMap.set_available(Services.PICO_TO_WAVE_ID, Reason.AVAILABLE)
            else:
                SettingsMap.set_available(Services.PICO_TO_WAVE_ID,
                                             Reason.NOT_AVAILABLE)
        except AbortException:
            reraise(*sys.exc_info())
        except Exception as e:
            MY_LOGGER.exception('')
            SettingsMap.set_available(Services.PICO_TO_WAVE_ID, Reason.BROKEN)
        ...
        try:
            ...
            if is_available:
                SettingsMap.set_available(Services.RESPONSIVE_VOICE_ID,
                                             Reason.AVAILABLE)
            else:
                SettingsMap.set_available(Services.RESPONSIVE_VOICE_ID,
                                             Reason.NOT_AVAILABLE)
            ...
            MY_LOGGER.exception('')
            SettingsMap.set_available(Services.RESPONSIVE_VOICE_ID, Reason.BROKEN)
        ...
        try:
            ...
            if is_available:
                SettingsMap.set_available(Services.SPEECH_DISPATCHER_ID,
                                             Reason.AVAILABLE)
            else:
                SettingsMap.set_available(Services.SPEECH_DISPATCHER_ID,
                                             Reason.NOT_AVAILABLE)
    ...
    @classmethod
    def load_engine(cls, engine_id: EngineType) -> None:
        try:
